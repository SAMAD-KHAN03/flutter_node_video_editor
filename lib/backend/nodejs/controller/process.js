...
function processVideo(obj, res, req) {
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker("/Users/samad/Documents/MyFlutterProjects/flutter_node_video_editor/lib/backend/nodejs/utils/FF.js", {
      workerData: {
        obj: obj,
      },
    });
    ...
    worker.on("exit", (code) => {
      if (code === 0) {
        ...
        res.end(
          JSON.stringify({
            message: `${obj.type} operation completed`,
            videoId,
          })
        );
        resolve();
