...
async function UploadToFirebase(obj) {
  try {
    const videopath = obj.outputPath;
    if (!fs.existsSync(videopath)) {
      throw new Error(`File not found: ${videopath}`);
    }
    ///Users/samad/Documents/MyFlutterProjects/govideoeditor/lib/backend/nodejs/storage/4d4ea94fb7187c753bcaa3ecb8307ffe061619d03fbaf89a790e41ec1dcfaa38-resized-49x49.mp4
...
async function download(obj) {
  ...
  const file = downloadbucket.file(`${type}/${videoId}.${mime}`);
  try {
    await file.download({
      destination:
